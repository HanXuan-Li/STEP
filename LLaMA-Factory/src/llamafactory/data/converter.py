    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        for turn_idx, message in enumerate(messages):
            if message[self.dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
                logger.warning_rank0("Invalid role tag (details omitted).")
                broken_data = True
                break
        ...
        if (not self.dataset_attr.ranking and len(aligned_messages) % 2 != 0) or (
            ...
            logger.warning_rank0("Invalid message count (details omitted).")
            broken_data = True
        ...
        if broken_data:
            ...
            if (
                ...
                logger.warning_rank0("Invalid ranking role tag (details omitted).")
                broken_data = True
