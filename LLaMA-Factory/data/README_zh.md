...
- [样例数据集](https://huggingface.co/datasets/llamafactory/demo_data)
...
- 本仓库不包含视频样例。请在 `dataset_info.json` 中使用中性名称（例如
  `<YOUR_VIDEO_DATASET>`）注册您自己的数据集。
...
- 本仓库不包含音频样例。请在 `dataset_info.json` 中使用中性名称（例如
  `<YOUR_AUDIO_DATASET>`）注册您自己的数据集。
